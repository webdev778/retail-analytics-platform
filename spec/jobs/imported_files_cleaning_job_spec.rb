...
RSpec.describe ImportedFilesCleaningJob, type: :job do
  ...
  let(:csv) { create(:csv, user: user, created_at: 5.month.ago) }
  ...
  context 'upload records 6' do
    ...
    it 'should destroy 1 record' do
      ...
      expect(InventoryDataUpload.all).to eq [finished_csv2, finished_csv3, finished_csv4, finished_csv5, finished_csv6]
    end
  end
end
